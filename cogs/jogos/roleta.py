...
class Roleta(commands.Cog):
    def __init__(self, bot):
        ...
        self.apostas = [] # Lista de dicionários: {'user': Member, 'valor': float, 'tipo': str}

    def get_limite(self, cargo):
    ...
    async def roleta(self, ctx):
        ...
        embed_abertura = disnake.Embed(
            ...
            description=f"O Chimpanzézio abriu a mesa! Vocês têm **30 segundos** para apostar.\n\n**Opções:**\n🔴 `vermelho` (2x)\n⚫ `preto` (2x)\n⚖️ `par`/`impar` (2x)\n🎯 `0 a 36` (36x)\n\n*Prêmios isentos de taxa!*",
        ...
        for aposta in self.apostas:
            ...
            if ganhou:
                # Lucro sem taxa!
                lucro_liquido = round((valor * multiplicador) - valor, 2)
                retorno_final = round(valor + lucro_liquido, 2)
                
                if user_db:
                    db.update_value(user_db['row'], 3, round(float(user_db['data'][2]) + retorno_final, 2))
                    if multiplicador == 36: await self.save_achievement(user_db, "filho_da_sorte")
                
                vencedores_txt += f"🎉 {jogador.mention} lucrou **{lucro_liquido:.2f} C** em `{aposta_em.upper()}`!\n"
            else:
    ...
    async def apostar(self, ctx, valor: float = None, aposta_em: str = None):
        ...
        cargo = user['data'][3]
        limite = self.get_limite(cargo)
        apostado_ja = sum(a['valor'] for a in self.apostas if a['user'] == ctx.author)
        
        if (apostado_ja + valor) > limite:
            restante = round(limite - apostado_ja, 2)
            return await ctx.send(f"🚫 {ctx.author.mention}, seu limite como **{cargo}** é **{limite} C**. Você só pode apostar mais **{max(0, restante)} C**.")
