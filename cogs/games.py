...
class BlackjackView(disnake.ui.View):
    ...
    async def atualizar_embed(self, inter=None):
        embed = disnake.Embed(title="🃏 MESA DE BLACKJACK (21)", color=disnake.Color.dark_purple())
        d_pontos = self.calcular_pontos(self.dealer_hand)
        embed.add_field(name="🏦 Dealer (Bot)", value=f"Mão: `{self.formatar_mao(self.dealer_hand, not self.terminado)}`\nPontos: `{'?' if not self.terminado else d_pontos}`", inline=False)
        for p_id in self.player_ids:
            p = self.players_data[p_id]
            status = "⏳" if p_id == self.player_ids[self.current_player_idx] and not self.terminado else "✅"
            if p["status"] == "estourou": status = "💥"
            if p["status"] == "parou": status = "✋"
            embed.add_field(name=f"{status} {p['member'].display_name}", value=f"Mão: `{self.formatar_mao(p['hand'])}`\nPontos: `{self.calcular_pontos(p['hand'])}` | Aposta: `{p['aposta']} C`", inline=True)
        if inter: await inter.response.edit_message(embed=embed, view=None if self.terminado else self)
        else: return embed

    @disnake.ui.button(label="Pedir (Hit)", style=disnake.ButtonStyle.green)
    async def hit(self, button, inter):
        if inter.author.id != self.player_ids[self.current_player_idx]: return await inter.send("❌ Não é sua vez!", ephemeral=True)
    ...
    async def double(self, button, inter):
        ...
        if p_id != self.player_ids[self.current_player_idx]: return await inter.send("❌ Não é sua vez!", ephemeral=True)
        p = self.players_data[p_id]
        u_db = db.get_user_data(str(p_id))
        if int(u_db['data'][2]) < p["aposta"]: return await inter.send("❌ Saldo insuficiente!", ephemeral=True)
    ...
    async def proximo_turno(self):
        ...
        if self.current_player_idx >= len(self.player_ids):
            self.terminado = True
            while self.calcular_pontos(self.dealer_hand) < 17: self.dealer_hand.append(self.deck.pop())
            await self.processar_resultados()

    async def processar_resultados(self):
        d_p = self.calcular_pontos(self.dealer_hand)
        res = f"🏦 **Dealer finalizou com {d_p} pontos.**\n\n"
        for p_id, p in self.players_data.items():
            p_p = self.calcular_pontos(p["hand"])
            u_db = db.get_user_data(str(p_id))
            if p_p > 21: res += f"❌ {p['member'].mention} estourou!\n"
            elif d_p > 21 or p_p > d_p:
                db.update_value(u_db['row'], 3, int(u_db['data'][2]) + (p["aposta"] * 2))
                res += f"💰 {p['member'].mention} ganhou `{p['aposta']*2} C`!\n"
            elif p_p == d_p:
                db.update_value(u_db['row'], 3, int(u_db['data'][2]) + p["aposta"])
                res += f"🤝 {p['member'].mention} empatou.\n"
            else: res += f"💀 {p['member'].mention} perdeu.\n"
